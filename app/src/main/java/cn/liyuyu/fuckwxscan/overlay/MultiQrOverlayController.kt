...
class MultiQrOverlayController(
    ...
    fun show(
        ...
        if (sourceWidth <= 0 || sourceHeight <= 0 || results.size < 2) {
            return false
        }
        ...
        val overlay = ActiveOverlay(view)
    ...
    private fun createLayoutParams(): WindowManager.LayoutParams {
        return WindowManager.LayoutParams(
            ...
            // Keep the scanned app as the focused window. The overlay remains touchable, so
            // its markers and cancel action still receive taps.
