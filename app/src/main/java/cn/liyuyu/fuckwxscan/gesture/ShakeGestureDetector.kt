 ...
 * A light pair is accepted only with a clearer reversal; a pair containing a strong peak keeps
 * the wider direction tolerance so existing gestures continue to work.
 *
...
class ShakeGestureDetector(
    ...
    private val strongPeakThresholdMps2: Float = DEFAULT_STRONG_PEAK_THRESHOLD_MPS2,
    ...
    private val gentleMaxOpposingCosine: Float = DEFAULT_GENTLE_MAX_OPPOSING_COSINE,
    ...
    init {
        ...
        require(strongPeakThresholdMps2 >= peakThresholdMps2)
        ...
        require(gentleMaxOpposingCosine in -1f..maxOpposingCosine)
    ...
    fun onSample(x: Float, y: Float, z: Float, eventTimeMs: Long): Boolean {
        ...
        if (!peakArmed) {
            val currentFirstPeak = firstPeak
            if (currentFirstPeak != null && magnitude > currentFirstPeak.magnitude) {
                firstPeak = Peak(x, y, z, magnitude, eventTimeMs)
            }
            return false
        }
        ...
        if (magnitude < requiredPeakThreshold) {
            if (expiredPeak != null) {
                recordDiagnostic(
                    stage = DiagnosticStage.EXPIRED,
                    sampleMagnitudeMps2 = magnitude,
        ...
        val requiredCosine = if (
            maxOf(previousPeak.magnitude, currentPeak.magnitude) >= strongPeakThresholdMps2
        ) {
            maxOpposingCosine
        } else {
            gentleMaxOpposingCosine
        }
        if (separationMs <= maxPeakSeparationMs && cosine <= requiredCosine
        ) {
            firstPeak = null
    ...
    companion object {
        const val DEFAULT_PEAK_THRESHOLD_MPS2 = 8f
        const val DEFAULT_SECOND_PEAK_THRESHOLD_MPS2 = 6f
        const val DEFAULT_STRONG_PEAK_THRESHOLD_MPS2 = 12f
        const val DEFAULT_RELEASE_THRESHOLD_MPS2 = 4f
        const val DEFAULT_MIN_PEAK_SEPARATION_MS = 100L
        const val DEFAULT_MAX_PEAK_SEPARATION_MS = 700L
        const val DEFAULT_MAX_OPPOSING_COSINE = -0.25f
        const val DEFAULT_GENTLE_MAX_OPPOSING_COSINE = -0.60f
